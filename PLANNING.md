...
| TCGCSV integration | 🟡 | In progress (2026-05-27): canonical-id foundation + `/api/tcgcsv` proxy + `src/pricing.js` client (Stage 1) + UI now reads raw market prices from TCGCSV with PC fallback (Stage 2). Variant resolver and PriceCharting removal still pending. |
